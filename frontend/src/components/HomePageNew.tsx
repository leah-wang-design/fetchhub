...
import { Image as ImageIcon, ChevronDown, ExternalLink, MessageSquare, Clock, Ellipsis, ArrowLeft, X, Plus } from 'lucide-react';
import type { PageSession, Screenshot, Comment, Workspace } from '../types';
...
export default function HomePage() {
  ...
  const [recentViewType, setRecentViewType] = useState<'page' | 'date'>('page');
  ...
  return (
    <div className="flex flex-col h-full bg-cream dark:bg-gray-900">
      <ModalComponent />
      <NavBar />
      <div className="flex-1 bg-cream dark:bg-gray-900 px-36 py-36">
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <div className="flex gap-3">
                  <span className={`flex-shrink-0 font-bold ${typography.body}`}>1</span>
                  <p className={typography.body}>Install the Chrome extension to capture screenshots of any webpage</p>
                </div>
                <div className="flex gap-3">
                  <span className={`flex-shrink-0 font-bold ${typography.body}`}>2</span>
                  <p className={typography.body}>Click and drag to select areas on the page where you want to add feedback</p>
                </div>
                <div className="flex gap-3">
                  <span className={`flex-shrink-0 font-bold ${typography.body}`}>3</span>
                  <p className={typography.body}>Add comments, reply to team members, and track feedback resolution</p>
                </div>
                <div className="flex gap-3">
                  <span className={`flex-shrink-0 font-bold ${typography.body}`}>4</span>
                  <p className={typography.body}>View all feedback in this dashboard, organized by page and timestamp</p>
                </div>
              </div>
            </div>
            ...
            <div className="pt-6">
              <a
                ...
                className={`${buttonStyles.link} inline-flex items-center gap-2`}
              >
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg border border-stone-200 dark:border-gray-700 p-6">
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div className="relative">
                  ...
                  >
                    <span>{selectedWorkspaceId ? workspaces.find(w => w.id === selectedWorkspaceId)?.name : 'All workspace'}</span>
                  ...
                  {isWorkspaceDropdownOpen && (
                    <>
                      ...
                      />
                      <div className="absolute left-0 top-full mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg z-20 min-w-[180px]">
                        <button
                          onClick={() => {
                ...
                <div className="relative">
                  ...
                  >
                  <span>{recentViewType === 'page' ? 'By page' : 'By comment'}</span>
                  <ChevronDown className="w-3 h-3" />
                </button>
                    ...
                    {sessions.slice(0, 5).map((session) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          navigate(`/gallery?url=${encodeURIComponent(session.page_url)}&workspace=${workspaceParam}&from=home`);
                        }}
                    ...
                    {recentComments.slice(0, 5).map((comment) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          navigate(`/gallery?url=${encodeURIComponent(comment.screenshot.page_url)}&workspace=${workspaceParam}&from=home`);
                        }}
                        className="w-full text-left p-4 bg-stone-50 dark:bg-gray-750 rounded-lg hover:bg-stone-100 dark:hover:bg-gray-700 transition-colors"
                      >
                                ...
                                <div className="absolute right-0 top-full mt-1 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-1 z-10">
                                  <button
                                    onClick={(e) => {
                                      ...
                                      setOpenCommentMenuId(null);
                                      const workspaceParam = comment.screenshot.workspace_id || 'default';
                                      navigate(`/gallery?url=${encodeURIComponent(comment.screenshot.page_url)}&workspace=${workspaceParam}&screenshotId=${comment.screenshot.id}&commentId=${comment.id}`);
                                    }}
                                    className="w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-stone-100 dark:hover:bg-gray-700 transition-colors"
                                  >
                                    View comment
